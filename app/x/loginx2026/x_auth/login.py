...
# Headers لتصفح الصفحات (document/navigation)
NAVIGATION_HEADERS = {
    **_COMMON_HEADERS,
...
class HTTPClient(curl_cffi.AsyncSession):
    ...
    async def request(self, method, url, *args, use_transaction_id=True, **kwargs):
        ...
        # Debug: show request details for API calls
        if 'api.x.com' in url:
            h = kwargs.get('headers', {})
            print(f'\n[DEBUG REQUEST] {method} {url}')
            print(f'  guest_token: {h.get("x-guest-token", "MISSING")}')
            print(f'  csrf_token: {h.get("x-csrf-token", "MISSING")}')
            print(f'  transaction_id: {h.get("x-client-transaction-id", "MISSING")[:30]}...')
            print(f'  sec-fetch-dest: {h.get("Sec-Fetch-Dest", h.get("sec-fetch-dest", "MISSING"))}')
            print(f'  sec-fetch-mode: {h.get("Sec-Fetch-Mode", h.get("sec-fetch-mode", "MISSING"))}')
            print(f'  sec-fetch-site: {h.get("Sec-Fetch-Site", h.get("sec-fetch-site", "MISSING"))}')
            print(f'  origin: {h.get("origin", "MISSING")}')
            body = kwargs.get('json', {})
            if body and 'subtask_inputs' in body:
                for si in body['subtask_inputs']:
                    sid = si.get('subtask_id', '?')
                    has_castle = 'castle_token' in str(si)
                    print(f'  subtask: {sid} (castle_token: {"yes" if has_castle else "no"})')
    ...
    def build_headers(self, authorization=True, csrf_token=True, extra_headers=None, json_content=False, api=False):
        headers = API_HEADERS.copy() if api else NAVIGATION_HEADERS.copy()
        if authorization:
...
class LoginFlow:
    ...
    async def _api_call(self, data, params=None):
        headers = self.http.build_headers(api=True, json_content=True, extra_headers={
    ...
    async def execute_subtasks(self):
        ...
        step_names = list(self.subtask_inputs.keys())
        data = {'flow_token': self.flow_token, 'subtask_inputs': subtask_inputs}
        try:
            response = await self._api_call(data)
        except Exception as e:
            raise Exception(
                f'فشل في الخطوة: {step_names}\n{e}\n\n'
                'حاول مرة أخرى بعد دقائق. إذا استمر الخطأ جرّب من IP مختلف أو استخدم login_with_cookies.'
            ) from e
        self._process_response(response)
    ...
    async def js_instrumentation(self):
        ...
        headers = self.http.build_headers(api=True, authorization=False, csrf_token=False)
...
async def x_login(username, password, email=None, cookies_dir="cookies"):
    ...
    # 0. كشف IP الحالي
    try:
        ip_resp = await http.get('https://api.ipify.org?format=json', headers={'accept': '*/*'}, use_transaction_id=False)
        ip = ip_resp.json().get('ip', '?')
        print(f'[0] IP: {ip}')
    except Exception:
        print(f'[0] IP: تعذر الكشف')
...
async def x_login_from_browser(cookies_dir="cookies", browser="chrome"):
    """
    استخراج كوكيز X مباشرة من المتصفح (Chrome/Edge/Firefox)
    يجب أن تكون مسجل دخول في المتصفح أولاً

    Args:
        cookies_dir: مجلد حفظ الكوكيز
        browser: نوع المتصفح ("chrome", "edge", "firefox", "brave")

    Returns:
        dict: الكوكيز المستخرجة
    """
    try:
        import browser_cookie3
    except ImportError:
        raise ImportError('مطلوب: pip install browser_cookie3')
    ...
    if browser not in browser_funcs:
        raise ValueError(f'متصفح غير مدعوم: {browser}. المتاح: {list(browser_funcs.keys())}')

    print(f'[*] استخراج كوكيز X من {browser}...')
    print(f'    (تأكد أن المتصفح مغلق أو أغلقه الآن)')

    try:
        cj = browser_funcs[browser](domain_name='.x.com')
    except Exception as e:
        raise Exception(f'فشل استخراج الكوكيز من {browser}: {e}')

    # استخراج auth_token و ct0
    cookies_dict = {}
    for cookie in cj:
        if cookie.domain and '.x.com' in cookie.domain:
    ...
    if not auth_token:
        raise Exception(f'auth_token غير موجود — تأكد أنك مسجل دخول في {browser} على x.com')

    print(f'    auth_token: {auth_token[:10]}...')
    print(f'    ct0: {ct0[:10]}...' if ct0 else '    ct0: سيتم جلبه')
    ...
    if not ct0:
        ...
        if not ct0:
            await http.close()
            raise Exception('فشل جلب ct0 — الكوكيز قد تكون منتهية')
        cookies_dict['ct0'] = ct0

    # حفظ الكوكيز
    username = 'browser_' + browser
    cookie_file = os.path.join(cookies_dir, f"{username}.json")
    playwright_cookies = _to_playwright_format(http.cookies, COOKIES_DOMAIN)
    with open(cookie_file, 'w', encoding='utf-8') as f:
        json.dump({"cookies": playwright_cookies, "origins": []}, f, ensure_ascii=False, indent=2)
    ...
    # حفظ في coo.txt
    _save_to_coo(ct0, auth_token, username, cookies_dir)

    await http.close()
    print(f'[+] تم استخراج الكوكيز بنجاح من {browser}!')
    return cookies_dict
