"""
نسخة محلية من x_auth بدون relative imports
"""
import os
import sys
import json
...
# استيراد الملفات الأخرى مباشرة
from app.x.modules.x_auth.castle import CastleToken
from app.x.modules.x_auth.transaction import ClientTransaction
from app.x.modules.x_auth.ui_metrics import solve_ui_metrics
...
START_FLOW_PAYLOAD = {
    ...
    "subtask_versions": {
        ...
        "generic_urt": 3, "in_app_notification": 1, "menu_dialog": 1,
...
# ============ Login Flow ============
class LoginFlow:
    def __init__(self, http: HTTPClient, castle: CastleToken):
        self.http = http
        self.castle = castle
    ...
    async def start_flow(self):
        ...
        payload = START_FLOW_PAYLOAD.copy()
        response = await self.http.post(
            'https://x.com/i/api/1.1/onboarding/task.json',
            json=payload,
            headers=self.http.build_headers(authorization=True, json_content=True)
        )
        data = response.json()
        self.flow_token = data.get('flow_token')
        self.subtasks = data.get('subtasks', [])
        print(f'[*] Flow token: {self.flow_token[:20]}...')

    async def execute_subtasks(self):
        """تنفيذ المهام المتراكمة"""
        payload = {
            'flow_token': self.flow_token,
            'subtask_inputs': self.subtask_inputs
        }
        response = await self.http.post(
            'https://x.com/i/api/1.1/onboarding/task.json',
            json=payload,
    ...
    async def sso_init(self):
        """مهمة SSO init"""
        self.subtask_inputs['LoginSSOInit'] = {
            'sso_init': {
                'link': 'next_link'
            }
        }
